      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid">
        <div className="grid w-full sm:space-x-[18px] space-y-[20px] sm:space-y-0 sm:grid-cols-2">
          <div className="grid w-full">
            ...
            {/* border div */}
            <div className="border-[1px] border-[#D0D0D0] mt-[23px]" />
            {/* input */}
...
function Label({ children }) {
  return (
    <label htmlFor={children} className="font-[400] text-[13px] text-[#4C4D4D]">
      {children}
    </label>
  );
}
...
function OptionSelection({ children, another = false }) {
  return (
    <div>
      {another ? (
        children
...
function Option({ children = "Small" }) {
...
function EditButton({ children = "Cancel", ...props }) {
  ...
  return (
    <button
      ...
      {...props}
      className={twMerge(
        "font-[500] text-[13px] text-[#009975] px-[32px] py-[12px] border-[2px] border-[#009975] rounded-[8px]",
        difBtn
      )}
    >
      {children}
