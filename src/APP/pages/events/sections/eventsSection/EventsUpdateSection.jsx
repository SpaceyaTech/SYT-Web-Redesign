      ...
      <div className="flex flex-row items-center">
        ...
        { topEventsCities && Array.isArray(topEventsCities.cities) ? (
            ...
            {topEventsCities.cities.map((city, index) => (
                <button key={index} id="dropdownDividerButton" data-dropdown-toggle="dropdownDivider" className="text-blue-700 underline font-medium text-s px-2 py-2.5 text-center inline-flex items-center transition duration-150 ease-in-out" type="button" onClick={() => {updateCityFilter(city); }}>
                  {city}
                  {' '}
            ...
            { cityFilter.city && 
            <span className="text-red-800 underline font-medium text-s px-2 py-2.5 text-center cursor-pointer" onClick={() => updateCityFilter('')}>Clear City</span>
            }
            </> 
        ) : ''}
      </div>
