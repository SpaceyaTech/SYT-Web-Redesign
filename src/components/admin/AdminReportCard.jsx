...
const AdminReportCard = ({ name = "Sales", amount = 80000, data = totalSalesData }) => {
  ...
  return (
    ...
    <div className="grid w-full  border-[1px] border-[#D0D0D0]  rounded-[12px] p-[20px] font-poppins">
      <div className="grid justify-start items-center w-full ">
        <span className="font-[500] uppercase text-[#7E8180]">Total {name}</span>
        <span className="text-[#009975] flex w-fit  text-[32px] font-[600] uppercase">KES {amount}</span>
      </div>
      <div className=" grid gap-[26px] mt-4">
        {data.map((e, index) => (
          <div key={index} className={twMerge("flex justify-between items-start text-[16px] h-[36px] border-b-[1px] border-[#D0D0D0]", e.name == "This year" ? "border-b-0" : "border-b-[1px]")}>
            <span className="font-[500] text-[#7E8180]  leading-[24px]">{e.name}</span>
...
const HighDemandCard = ({ hignDemandData = firstHighDemandData, bigCard = false, name }) => {
  const dataTableData = { Name: "Name", ItemsSold: "Items Sold", UnitCost: "UNIT COST(KES)", TotalCost: "TOTAL SALES(KES)" };
  return (
    <>
      <div className='grid w-full  border-[1px] border-[#D0D0D0] rounded-[12px]  font-poppins overflow-x-auto'>
        ...
        {hignDemandData.map((e, index) => (
          <div key={index} className='flex items-center justify-center h-[52px]  even:bg-[#F9FAFB] uppercase  font-[500] text-[14px] text-[#6B7280]'>
            <span className='w-1/2 px-[24px]'>{e.city}</span>
            <span className={twMerge('w-1/2 px-[24px] font-[600] text-[#4C4D4D]', bigCard && "font-[500]")}>{e.items}</span>
            {
              bigCard === true &&
              <>
...
const TotalVisitsCustomCard = ({ dataOne = totalVisitInnerCardData, dataTwo = totalVisitInnerCardDataTwo }) => {
  return (
    <>
      <div className='grid w-full  border-[1px] border-[#D0D0D0]  rounded-[12px] p-[20px] font-poppins'>
        <div className='flex justify-start '>
          <div className='grid h-[97px]  justify-center items-center '>
            <span className='font-[500] uppercase text-[#7E8180] '>TOTAL VISITS</span>
            <span className='text-[#4C4D4D] flex  text-[32px] font-[600]  uppercase'>100,000</span>
          </div>
        </div>
        {/* border */}
        ...
        <div className='grid  justify-center grid-cols-3 gap-[32px] items-center py-[8px] '>
          {dataOne.map((e, index) => (
            <div key={index} className=' h-[60px] w-[84px] text-start    grid items-center justify-center text-[14px] font-[500] text-[#7E8180]'>
              <span className='tracking-wider' >{e.name}</span>
              <span className='font-[600] text-[#4C4D4D] text-[24px]' >{e.number}</span>
            </div>
          ))}
        </div>
        {/* border */}
...
const ChartComponent = ({ title = "All Orders", number = 500, persontage = "10%" }) => {
  const data = [
...
const InventoryDataTwo = [
  { id: "ORD-101", email: 'ian@gmail.com', item: "SYT Beanie", noOfId: "No. of items: 350", unitCost: "Unit Cost: KES 350", status: "Complete", },
  { id: "ORD-101", email: 'ian@gmail.com', item: "SYT Beanie", noOfId: "No. of items: 5", unitCost: "Unit Cost: KES 1000", status: "Unfulfilled" },
  { id: "ORD-101", email: 'ian@gmail.com', item: "SYT Beanie", noOfId: "No. of items: 350", unitCost: "Unit Cost: KES 350", status: "Pending" },
  { id: "ORD-101", email: 'ian@gmail.com', item: "SYT Beanie", noOfId: "No. of items: 350", unitCost: "Unit Cost: KES 350", status: "Complete" },
  { id: "ORD-101", email: 'ian@gmail.com', item: "SYT Beanie", noOfId: "No. of items: 350", unitCost: "Unit Cost: KES 350", status: "Pending" },
  { id: "ORD-101", email: 'ian@gmail.com', item: "SYT Beanie", noOfId: "No. of items: 350", unitCost: "Unit Cost: KES 350", status: "Complete" },
];
                      ...
                      <>
                        <tr className=' text-[14px] text-[#7E8180] font-[500] align-middle even:bg-[#F9FAFB]'>
                          ...
                          <td className="h-[52px] px-6 text-[12px] font-[500px]">
                            <span className={twMerge("bg-transparent py-[2px] px-[10px] rounded-[10px]", e.status === "Complete" && "bg-[#D1FAE5] text-[#065F46]", e.status === "Unfulfilled" && "bg-[#FEE2E2] text-[#991B1B]", e.status === "Pending" && "bg-[#FEF3C7] text-[#92400E]")}>
                              {e.status}
                            </span>
                          </td>
                          <td className="h-[52px] px-6 text-right ">
                            <span className='text-[#009975] cursor-pointer'>View </span>
                          </td>
                        </tr >
                      </>
                    ))}
